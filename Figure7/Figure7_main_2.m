...
% version: 2021.11.16
% OAMP-MMV-SSL + OAMP-MMV-ASL for thegma2 = (eq.(24), 1e-4, 1e-2, 1)
% ADEP and BER vs M
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc; clear;
tic
...
testall = 2;
thegma2_set = char('eq.24','1e-4','1e-2','1');
ADEP = zeros(length(M_set), length(thegma2_set), testall);     
BER = zeros(length(M_set), length(thegma2_set), testall);
NMSE = zeros(length(M_set), length(thegma2_set), testall);

%% Simulation
for i_test = 1:size(thegma2_set,1)
    for alg_sel = 1:testall
        thegma2 = thegma2_set(i_test, :);
        for sim = 1:Nsim
            for mm = 1:length(M_set)
                ...
                switch alg_sel
                    ...
                    if i_test == 1
                        ...
                        [Xhat_tmp, lambda] = OAMP_MMV_SSL_test_para(Y, S_wave, damp, Iiter, prior, L, 'thegma2', str2double(thegma2));
                    end
                    Xhat(:,:) = Xhat_tmp(:,:,end);
                    ...
                    if i_test == 1
                        ...
                        [Xhat_tmp, lambda] = OAMP_MMV_ASL_test_para(Y, S_wave, damp, Iiter, prior, L, var_type, 'thegma2', str2double(thegma2));
                    end
                    Xhat(:,:) = Xhat_tmp(:,:,end);
                    act_hat(mean(lambda,2) > ad_th) = 1;   
                end
                ...
                if mod(sim,100) == 0
                    fprintf('%s, sim = %d, M = %d, thegma2 = %s, ADEP = %4.8f, BER = %4.5f, NMSE = %7.9f\n', ...
                            method(alg_sel,:), sim, M, thegma2, ADEP(mm,i_test,alg_sel)/sim, BER(mm,i_test,alg_sel)/sim, 10*log10(NMSE(mm,i_test,alg_sel)/sim));
                end
            end
        end
        toc
...
legend('OAMP-MMV-SSL, $(\sigma^2)^0 = eq.(24)$','OAMP-MMV-ASL, $(\sigma^2)^0 = eq.(24)$',...
       'OAMP-MMV-SSL, $(\sigma^2)^0 = 10^{-4}$','OAMP-MMV-ASL, $(\sigma^2)^0 = 10^{-4}$',...
       'OAMP-MMV-SSL, $(\sigma^2)^0 = 10^{-2}$','OAMP-MMV-ASL, $(\sigma^2)^0 = 10^{-2}$',...
       'OAMP-MMV-SSL, $(\sigma^2)^0 = 10^{0}$','OAMP-MMV-ASL, $(\sigma^2)^0 = 10^{0}$',...
