...
MSE_se = zeros(Iiter,testall, length(M_set)); 
BER_se = zeros(testall, length(M_set)); 
ADEP = zeros(testall, length(M_set)); 
BER_SE_total = zeros(testall,length(M_set));
for mm = 1:length(M_set)
    for alg_sel_se=1:testall   
        ...
        for n=1:N
            ...
            ADEP(alg_sel_se, mm) = ADEP(alg_sel_se, mm) + sum(abs(act_flag-act_hat_se))/K;
            if isempty(id_act_t)
                Err_bit = Ka*T*log2(L);
            else
        ...
        fprintf('SE: %s, M = %d, SNR = %d dB, T=%d, ADEP=%3.4f, BER=%4.7f, MSE = %7.9f\n', ...
              method(alg_sel_se,:), M, SNR_dB, T, ADEP(alg_sel_se, mm)/N, BER_se(alg_sel_se, mm)/N, 10*log10(MSE_se(Iiter,alg_sel_se,mm)));
    end
end
toc
...
figure;
Iiter = 50;
semilogy(M_set,SE.BER_se(1,:),'m-.<','LineWidth',LineWidth,'MarkerSize',MarkerSize); hold on;grid on;
semilogy(M_set,SE.BER_se(2,:),'m-.>','LineWidth',LineWidth,'MarkerSize',MarkerSize); hold on;grid on;
